...
var SYMBOL_FILTERS = {
  // Forex
  EURUSD:{ adxTrend:25, adxNormal:10, rsiMax:72, rsiMaxTrend:74, rsiMin:28, rsiMinTrend:26, noVol:true,  consensus:2, slMult:1.5, tpMult:2.0 },
  GBPUSD:{ adxTrend:25, adxNormal:10, rsiMax:72, rsiMaxTrend:74, rsiMin:28, rsiMinTrend:26, noVol:true,  consensus:2, slMult:1.5, tpMult:2.0 },
  USDJPY:{ adxTrend:25, adxNormal:10, rsiMax:76, rsiMaxTrend:78, rsiMin:24, rsiMinTrend:22, noVol:true,  consensus:2, slMult:1.5, tpMult:2.0 },
  GBPJPY:{ adxTrend:25, adxNormal:10, rsiMax:78, rsiMaxTrend:80, rsiMin:22, rsiMinTrend:20, noVol:true,  consensus:2, slMult:1.5, tpMult:2.0 },
  AUDUSD:{ adxTrend:25, adxNormal:10, rsiMax:72, rsiMaxTrend:74, rsiMin:28, rsiMinTrend:26, noVol:true,  consensus:2, slMult:1.5, tpMult:2.0 },
  USDCAD:{ adxTrend:25, adxNormal:10, rsiMax:72, rsiMaxTrend:74, rsiMin:28, rsiMinTrend:26, noVol:true,  consensus:2, slMult:1.5, tpMult:2.0 },
  USDCHF:{ adxTrend:25, adxNormal:10, rsiMax:72, rsiMaxTrend:74, rsiMin:28, rsiMinTrend:26, noVol:true,  consensus:2, slMult:1.5, tpMult:2.0 },
  NZDUSD:{ adxTrend:25, adxNormal:10, rsiMax:72, rsiMaxTrend:74, rsiMin:28, rsiMinTrend:26, noVol:true,  consensus:2, slMult:1.5, tpMult:2.0 },
  // Metals
  XAUUSD:{ adxTrend:30, adxNormal:10, rsiMax:71, rsiMaxTrend:75, rsiMin:29, rsiMinTrend:25, noVol:true,  consensus:2, slMult:1.8, tpMult:2.5 },
  XAGUSD:{ adxTrend:30, adxNormal:10, rsiMax:71, rsiMaxTrend:75, rsiMin:29, rsiMinTrend:25, noVol:true,  consensus:2, slMult:1.8, tpMult:2.5 },
  // Oil
  USOIL: { adxTrend:28, adxNormal:10, rsiMax:71, rsiMaxTrend:74, rsiMin:29, rsiMinTrend:26, noVol:true,  consensus:2, slMult:1.8, tpMult:2.5 },
  UKOIL: { adxTrend:28, adxNormal:10, rsiMax:71, rsiMaxTrend:74, rsiMin:29, rsiMinTrend:26, noVol:true,  consensus:2, slMult:1.8, tpMult:2.5 },
  WTIUSD:{ adxTrend:28, adxNormal:10, rsiMax:71, rsiMaxTrend:74, rsiMin:29, rsiMinTrend:26, noVol:true,  consensus:2, slMult:1.8, tpMult:2.5 },
  BRNUSD:{ adxTrend:28, adxNormal:10, rsiMax:71, rsiMaxTrend:74, rsiMin:29, rsiMinTrend:26, noVol:true,  consensus:2, slMult:1.8, tpMult:2.5 },
  // Crypto
  BTCUSD:{ adxTrend:30, adxNormal:10, rsiMax:70, rsiMaxTrend:73, rsiMin:30, rsiMinTrend:27, noVol:false, consensus:3, slMult:2.0, tpMult:3.0 },
  ETHUSD:{ adxTrend:30, adxNormal:10, rsiMax:70, rsiMaxTrend:73, rsiMin:30, rsiMinTrend:27, noVol:false, consensus:3, slMult:2.0, tpMult:3.0 },
  SOLUSD:{ adxTrend:30, adxNormal:10, rsiMax:70, rsiMaxTrend:73, rsiMin:30, rsiMinTrend:27, noVol:false, consensus:3, slMult:2.0, tpMult:3.0 },
  XRPUSD:{ adxTrend:30, adxNormal:10, rsiMax:70, rsiMaxTrend:73, rsiMin:30, rsiMinTrend:27, noVol:false, consensus:3, slMult:2.0, tpMult:3.0 },
  BNBUSD:{ adxTrend:30, adxNormal:10, rsiMax:70, rsiMaxTrend:73, rsiMin:30, rsiMinTrend:27, noVol:false, consensus:3, slMult:2.0, tpMult:3.0 },
};

function getSymbolFilters(sym) {
  return SYMBOL_FILTERS[sym] || { adxTrend:25, adxNormal:10, rsiMax:72, rsiMaxTrend:74, rsiMin:28, rsiMinTrend:26, noVol:true, consensus:2 };
}
...
async function checkSignal(sym, consensus, cooldownMin) {
  try {
  var symF = getSymbolFilters(sym);
  if (symF.consensus) consensus = symF.consensus;
  ...
  if (dir==='BUY'  && rsi > rsiMax) { st.stats.lastFilter='RSI ipercomprato ('+rsi.toFixed(1)+')'; return; }
  if (dir==='SELL' && rsi < rsiMin) { st.stats.lastFilter='RSI ipervenduto ('+rsi.toFixed(1)+')'; return; }

  // ── 4. MACD - allineato con direzione ──
  var macdM15 = calcMACD(st.candles, 12, 26, 9);
  if (macdM15) {
    var ml = macdM15.macd, ms = macdM15.signal;
    if (dir==='BUY'  && ml < ms) { st.stats.lastFilter='MACD contro BUY'; return; }
    if (dir==='SELL' && ml > ms) { st.stats.lastFilter='MACD contro SELL'; return; }
  }
  ...
  // ── 6. ADX - solo mercati completamente piatti ──
  if (trendLevel==='RANGE') {
    st.stats.lastFilter='Mercato piatto (ADX '+trendInfo.adx.toFixed(1)+' < 10)'; return;
  }
  ...
  var lastVol = c[len-1].vol||0;
  var volPct  = avgVol>0&&lastVol>0 ? Math.round(lastVol/avgVol*100) : 0;
  var atr0    = calcATR(st.candles,14)[len-2]||0;
  var srCheck = calcSR(st.candles, 50);
  var obCheck = detectOrderBlocks(st.candles);
  var fvgCheck = detectFVG ? detectFVG(st.candles) : [];
  ...
  var atr = atrArr[atrArr.length-2]||0;
  var symF3 = getSymbolFilters(sym);
  var slMult = symF3.slMult||1.5;
  var tpMult = slMult*2;
  var slDist = Math.max(atr*slMult, price*0.005);
  var sl = dir==='BUY' ? (price-slDist).toFixed(dec) : (price+slDist).toFixed(dec);
  var tp = dir==='BUY' ? (price+slDist*2).toFixed(dec) : (price-slDist*2).toFixed(dec);
  ...
  // Order Blocks
  var obs = detectOrderBlocks(st.candles);
  var nearOBs = getNearestOB(price, obs, atr);
  var buyOBs  = nearOBs.filter(function(ob){return ob.type==='BUY'&&ob.high<price;});
  var sellOBs = nearOBs.filter(function(ob){return ob.type==='SELL'&&ob.low>price;});
  var obConfirm = (dir==='BUY'&&buyOBs.length>0)||(dir==='SELL'&&sellOBs.length>0);

  var srText =
    (res?'Resistenza: '+res.price.toFixed(dec)+' (str:'+res.str+')'+nl:'')+
    (sup?'Supporto: '+sup.price.toFixed(dec)+' (str:'+sup.str+')'+nl:'')+
    (sellOBs.length?'OB Ribassista: '+sellOBs[0].low.toFixed(dec)+'-'+sellOBs[0].high.toFixed(dec)+nl:'')+
    (buyOBs.length?'OB Rialzista: '+buyOBs[0].low.toFixed(dec)+'-'+buyOBs[0].high.toFixed(dec)+nl:'');

  // Lot sizes
  var lots = [100,250,500,1000].map(function(b){
    return b+'EUR='+calcLotSize(sym,b,3,slDist)+' lot';
  }).join(' | ');

  // MSG 1 - immediate entry
  var msg1 =
    '[ST-EA] <b>'+dir+'</b> '+name+nl+
    '<b>Prezzo:</b> '+price.toFixed(dec)+nl+
    '<b>SL:</b> '+sl+' | <b>TP:</b> '+tp+nl+
    '<b>R:R:</b> 1:2 | <b>Lot:</b> '+calcLotSize(sym,500,3,slDist)+' lot (500EUR)'+nl+
    (obConfirm?'OB Confermato!'+nl:'')+
    '<b>ENTRA ORA!</b>';
  ...
  if (ok) {
    ...
    // MSG 2 - confirmation after 3 minutes
    setTimeout(async function(){
      var msg2 =
        '[ST-EA] CONFERMA <b>'+dir+'</b> '+name+nl+nl+
        '<b>Filtri:</b>'+nl+
  ...
  } catch(e) { console.error('checkSignal '+sym+': '+e.message); }
}
...
async function checkPreSignal(sym, consensus) {
  ...
  if (m15Dir===h1Dir && adx>=10 && (avgVol===0||lastVol===0||lastVol>=avgVol*0.7)) {
